...
import { getFirestore, collection, addDoc, onSnapshot, updateDoc, doc, deleteDoc, serverTimestamp, query, orderBy, where } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        listenForUserProblems(); // Start listening for ONLY the user's problems
    } else {
        try {
...
// --- Real-time Problems Listener for ONLY the current user ---
function listenForUserProblems() {
    console.log("Setting up real-time listener for problems submitted by user:", currentUserId);
    
    try {
        const problemsCollectionRef = collection(db, `artifacts/${YOUR_CUSTOM_APP_ID}/public/data/integralProblems`);
        
        // IMPORTANT: Query only problems submitted by the current user
        const q = query(problemsCollectionRef, where("submittedBy", "==", currentUserId));
        
        onSnapshot(q, (snapshot) => {
            console.log("Data received:", snapshot.docs.length, "problems from user", currentUserId);
            ...
            problemsList.innerHTML = `<p class="text-center text-red-600 dark:text-red-400">Failed to load your problems. Error: ${error.message}</p>`;
            loadingOverlay.classList.add('hidden');
        });
...
function updateProblemsCount() {
    ...
    if (countDisplay) {
        countDisplay.textContent = `Your problems: ${totalProblems}`;
    }
}

// --- Apply Sorting and Render with Pagination ---
function applyAndRenderSorting() {
    console.log("applyAndRenderSorting called. Current page:", currentPage, "Items per page:", itemsPerPage, "Your problems:", allProblems.length);
    ...
    if (problems.length === 0) {
        problemsList.innerHTML = `
            <div class="text-center py-12">
                ...
                <p class="text-gray-500 dark:text-gray-400 text-lg mb-4 font-serif">You haven't submitted any problems yet!</p>
                <p class="text-gray-400 dark:text-gray-500 text-sm font-serif">Use the form above to submit your first integral problem.</p>
            </div>`;
        return;
    }
...
function createProblemCard(problem) {
    ...
    // Right side: Edit and Delete buttons (only for owner - but all problems shown are owner's)
    const rightActions = document.createElement('div');
    rightActions.className = "flex items-center space-x-3";
    
    // Since we're only showing user's own problems, we can always show edit/delete buttons
